...
class ParameterMapper:
    ...
    def can_map(self, source_param: np.ndarray, target_param: np.ndarray) -> bool:
        ...
        return target_size <= source_size <= target_size * 2

    def map_parameters(
        self, source_param: np.ndarray, target_param: np.ndarray, method: str = "average"
    ) -> np.ndarray:
        ...
        if source_size > target_size:
            ...
            if method == "truncate":
                ...
                reshaped = source_param.flatten()[: block_size * target_size].reshape(
    ...
    def align_models(
        self, source_params: dict[str, np.ndarray], target_param_names: list[str]
    ) -> dict[str, np.ndarray]:
...
class HeterogeneousAggregator:
    ...
    def get_shared_parameters(
        self, baseline_param_names: list[str], client_param_names: list[str]
    ) -> list[str]:
    ...
    def normalize_client_update(
        self, client_update: dict[str, Any], baseline_param_names: list[str]
    ) -> dict[str, np.ndarray]:
        ...
        aligned = {}
        shared_params = self.get_shared_parameters(baseline_param_names, list(raw_params.keys()))
    ...
    def _weighted_average(self, values: list[np.ndarray], weights: np.ndarray) -> np.ndarray:
    ...
    def _hybrid_aggregate(self, values: list[np.ndarray], weights: np.ndarray) -> np.ndarray:
...
class BaselineModelManager:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.baseline_metadata: ModelMetadata | None = None
    ...
    def register_variant(self, model: Any, model_id: str) -> ModelMetadata:
        ...
        compatibility = (
            self.baseline_metadata.get_compatibility(metadata) if self.baseline_metadata else 0.0
        )
        self.logger.info(
    ...
    def can_aggregate_with(self, model_id: str) -> tuple[bool, float]:
        ...
        compatibility = (
            self.baseline_metadata.get_compatibility(self.model_variants[model_id])
            if self.baseline_metadata
            else 0.0
        )
