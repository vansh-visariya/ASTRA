...
def compute_similarity(a: np.ndarray, b: np.ndarray) -> dict[str, float]:
    ...
    return {"cosine": float(cosine), "euclidean": float(euclidean), "mse": float(mse)}


class MetricsTracker:
    """Track and log metrics during training."""

    def __init__(self, experiment_id: str, log_dir: Any, config: dict[str, Any]):
    ...
    def _init_loggers(self) -> None:
        ...
        self.jsonl_logger = JSONLLogger(self.log_dir / "metrics.jsonl")
    ...
    def log_metrics(self, step: int, metrics: dict[str, Any]) -> None:
        ...
        """
        entry = {"experiment_id": self.experiment_id, "step": step, **metrics}
...
def compute_attack_metrics(
    pre_attack_accuracy: float, post_attack_accuracy: float, recovery_step: int
) -> dict[str, Any]:
