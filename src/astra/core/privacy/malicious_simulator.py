...
class MaliciousSimulator:
    ...
    def inject_attack(self, gradient: np.ndarray, client_id: str) -> np.ndarray:
        """
    ...
    def simulate_label_flip(self, labels: torch.Tensor, flip_ratio: float = 0.5) -> torch.Tensor:
    ...
    def compute_attack_impact(
        self, original_accuracy: float, attacked_accuracy: float
    ) -> dict[str, float]:
        """
