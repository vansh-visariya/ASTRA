...
class TrainingGroup:
    ...
    def add_client(self, client_id: str, client_info: dict | None = None) -> None:
    ...
    def add_update(self, client_id: str, update: dict) -> bool:
        ...
        logger.info(
            f"[ADD-UPDATE] Client {client_id} in group {self.group_id}. "
            f"Group clients: {list(self.clients.keys())}"
        )
        if client_id not in self.clients:
            logger.warning(
                f"\u26a0\ufe0f  Client {client_id} NOT in group {self.group_id}"
                f" clients. Available: {list(self.clients.keys())}"
            )
        if client_id in self.clients:
