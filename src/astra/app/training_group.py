...
class TrainingGroup:
    ...
    def add_client(self, client_id: str, client_info: Optional[dict] = None) -> None:
    ...
    def add_update(self, client_id: str, update: dict) -> bool:
        ...
        logger.info(
            f"[ADD-UPDATE] Client {client_id} in group {self.group_id}. Group clients: {list(self.clients.keys())}"
        )
        if client_id not in self.clients:
            logger.warning(
                f"⚠️  Client {client_id} NOT in group {self.group_id} clients. Available: {list(self.clients.keys())}"
            )
        if client_id in self.clients:
            ...
            logger.info(
                f"✓ METRICS STORED: Client {client_id} in group {self.group_id} | acc={train_acc:.4f}, loss={train_loss:.4f}"
            )

        self.pending_updates.append(
            {"client_id": client_id, "update": update, "timestamp": time.time()}
        )
    ...
    def get_window_status(self) -> dict:
        ...
        return {
            ...
            "trigger_reason": "size"
            if len(self.pending_updates) >= self.window_config.window_size
            else ("time" if elapsed >= self.window_config.time_limit else "waiting"),
        }
