...
def create_auth_router(platform: FLPlatformIntegration) -> APIRouter:
    ...
    @router.get("/users")
    async def list_users(role: str | None = None, admin: dict = Depends(require_admin)):
...
def create_group_join_router(platform: FLPlatformIntegration) -> APIRouter:
    ...
    async def request_join_group(
        request: JoinGroupRequest, current_user: dict = Depends(require_client)
    ):
        """Request to join a group (client request)."""
        result = platform.request_join_group(
            user_id=current_user["user_id"], group_id=request.group_id, metadata=request.metadata
        )
    ...
    async def get_join_requests(group_id: str | None = None, admin: dict = Depends(require_admin)):
    ...
    async def reject_join_request(request: RejectJoinRequest, authorization: str = Header(None)):
        ...
        result = platform.reject_join_request(
            token=token, request_id=request.request_id, reason=request.reason
        )
...
def create_notification_router(platform: FLPlatformIntegration) -> APIRouter:
    ...
    async def get_notifications(
        limit: int = 50, unread_only: bool = False, current_user: dict = Depends(get_current_user)
    ):
        """Get user notifications."""
        notifications = platform.get_notifications(
            user_id=current_user["user_id"], limit=limit, unread_only=unread_only
        )
...
def create_trust_router(platform: FLPlatformIntegration) -> APIRouter:
    ...
    async def get_all_trust_scores(
        group_id: str | None = None, admin: dict = Depends(require_admin)
    ):
    ...
    @router.post("/scores")
    async def update_trust_score(request: TrustScoreUpdate, admin: dict = Depends(require_admin)):
...
def create_recommendation_router(platform: FLPlatformIntegration) -> APIRouter:
    ...
    async def recommend_model(
        request: ClientMetadataRequest, current_user: dict = Depends(get_current_user)
    ):
    ...
    @router.get("/history")
    async def get_recommendation_history(limit: int = 50, admin: dict = Depends(require_admin)):
    ...
    async def get_unified_recommendations(
        request: ModelRecommendationRequest, current_user: dict = Depends(get_current_user)
    ):
        """Get unified recommendations (Gemini + builtin + custom HF models)."""
        from astra.app.model_recommender import metadata_from_dict
        ...
        recommendations = get_platform_integration().recommendation_service.get_all_recommendations(
            metadata=metadata, builtin_models=builtin_models, count=5
        )
    ...
    async def add_huggingface_model(
        request: AddHuggingFaceModelRequest, current_user: dict = Depends(get_current_user)
    ):
        ...
        result = get_platform_integration().recommendation_service.add_huggingface_model(
            model_url=request.model_url, use_peft=request.use_peft, peft_config=peft_config
        )
    ...
    @router.get("/builtin")
    async def get_builtin_models(current_user: dict = Depends(get_current_user)):
...
def create_inference_router(platform: FLPlatformIntegration) -> APIRouter:
    ...
    @router.post("/predict")
    async def predict(request: InferenceRequest, current_user: dict = Depends(get_current_user)):
        ...
        try:
            ...
            result = platform.run_inference(input_data=input_data, method=request.method)
    ...
    @router.get("/methods")
    async def get_methods(current_user: dict = Depends(get_current_user)):
...
def setup_extended_api(app, config: dict[str, Any]) -> FLPlatformIntegration:
