...
class FLPlatformIntegration:
    ...
    def _init_recommendations(self):
        ...
        if api_key:
            self.recommendation_service: ModelRecommendationService = init_recommendation_service(
                api_key
            )
    ...
    def require_role(self, token: str, allowed_roles: list[str]) -> dict[str, Any] | None:
    ...
    def request_join_group(
        ...
        for admin in admin_users:
            self.notification_service.notify_join_request(
                ...
                request_data={"user_id": user_id, "username": username, "metadata": metadata or {}},
            )

        return {"success": True, "request_nonce": nonce, "status": "pending"}

    def get_join_requests(
        self, token: str | None, group_id: str | None = None
    ) -> list[dict[str, Any]]:
    ...
    def approve_join_request(
        ...
        success = self.auth_manager.join_request_manager.approve_request(
            request_id, admin_payload["user_id"], secure_join_token
        )
    ...
    def reject_join_request(
        self, token: str, request_id: int, reason: str | None = None
    ) -> dict[str, Any]:
        ...
        success = self.auth_manager.join_request_manager.reject_request(
            request_id, admin_payload["user_id"]
        )
    ...
    def get_user_join_status(self, user_id: int, group_id: str) -> dict | None:
    ...
    def update_trust_score(
        self, user_id: int, group_id: str, score: float, quarantined: bool = False
    ):
        """Update trust score."""
        self.auth_manager.trust_score_manager.update_trust_score(
            user_id, group_id, score, quarantined
        )

    def get_all_trust_scores(self, group_id: str | None = None) -> dict[int, dict]:
        """Get all trust scores."""
        return self.auth_manager.trust_score_manager.get_all_trust_scores(group_id)

    # ==================== Model Recommendation Methods ====================

    def recommend_model(self, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    def run_inference(self, input_data: Any, method: str = "server_side") -> dict[str, Any]:
        """Run inference."""
        try:
...
_platform_integration: FLPlatformIntegration | None = None
