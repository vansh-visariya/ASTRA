...
class GroupManager:
                ...
                for g in db_groups:
                    ...
                    try:
                        ...
                        if os.path.exists(latest_path):
                            ...
                            self.logger.info(
                                f"Checkpoint found for {gid}: v{disk_version} (acc={checkpoint.get('accuracy', 0):.4f})"
                            )
                    except Exception as e:
                        self.logger.warning(f"Could not load checkpoint for group {gid}: {e}")

                    self.logger.info(
                        f"Restored group from DB: {gid} (status={group.status}, clients={len(group.clients)}, rounds={group.completed_rounds})"
                    )
    ...
    def log_event(self, event_type: str, message: str, group_id: Optional[str] = None, details: Optional[dict] = None):
    ...
    def get_logs(
        self, limit: int = 100, event_type: Optional[str] = None, group_id: str = None
    ) -> list[dict]:
        """Get recent logs."""
        with self.lock:
    ...
    def register_client(self, client_id: str, group_id: str, client_info: Optional[dict] = None) -> bool:
