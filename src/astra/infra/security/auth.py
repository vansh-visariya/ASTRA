...
class TokenManager:
    ...
    def validate_join_token(self, token: str) -> bool:
        ...
        with self.user_db._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT expires_at FROM used_tokens WHERE token_hash = ?", (token_hash,))
            row = cursor.fetchone()
...
class JoinRequestManager:
    ...
    def approve_request(self, request_id: int, resolved_by: int, token: str) -> bool:
    ...
    def get_user_request_status(self, user_id: int, group_id: str) -> dict | None:
