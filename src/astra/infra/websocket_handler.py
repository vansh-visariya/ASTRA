        ...
        while True:
            ...
            if message.get("type") == "register":
                ...
                try:
                    ...
                    logger.info(
                        f"[UPDATE-RECV] Client {client_id}:"
                        f" acc={received_meta.get('train_accuracy', 0):.4f},"
                        f" loss={received_meta.get('train_loss', 0):.4f}"
                    )
                    ...
                    logger.info(
                        f"[UPDATE] Processing update for client"
                        f" {client_id} in group {group.group_id}"
                    )
                    ...
                    logger = logging.getLogger(__name__)
                    logger.info(
                        f"[UPDATE] Client {client_id} in group"
                        f" {group.group_id}: acc={acc:.4f},"
                        f" loss={loss:.4f}"
                    )
                ...
                try:
                    ...
                    if client_id in group.clients:
                        ...
                        logger.debug(
                            f"Updated client {client_id} metrics:"
