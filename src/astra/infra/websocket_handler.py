        ...
        while True:
            ...
            if message.get("type") == "register":
                    ...
                    if not already_registered and not has_approved_join and not token_valid:
                        ...
                        try:
                            ...
                            if success:
                                ...
                                logger.info(
                                    f"[REGISTER] Client {client_id} registered. Group now has {len(group.clients)} clients: {list(group.clients.keys())}"
                                )
                ...
                try:
                    ...
                    logger.info(
                        f"[UPDATE-RECV] Client {client_id}: acc={received_meta.get('train_accuracy', 0):.4f}, loss={received_meta.get('train_loss', 0):.4f}"
                    )
                    ...
                    if not group:
                        ...
                        await websocket.send_json(
                            {"status": "rejected", "reason": "group_not_found"}
                        )
                        continue
                    ...
                    if not group.is_training:
                        ...
                        await websocket.send_json(
                            {"status": "rejected", "reason": "training_not_started"}
                        )
                        continue

                    logger.info(
                        f"[UPDATE] Processing update for client {client_id} in group {group.group_id}"
                    )
                    update_payload = fl_server.group_manager.normalize_update(
                        message.get("update", {})
                    )
                    update_result = fl_server.group_manager.process_client_update(
                        client_id, update_payload
                    )
                    ...
                    logger = logging.getLogger(__name__)
                    logger.info(
                        f"[UPDATE] Client {client_id} in group {group.group_id}: acc={acc:.4f}, loss={loss:.4f}"
                    )
                    fl_server.group_manager.log_event(
                        "client_update",
                        f"Client {client_id} sent update",
                        group.group_id,
                        {"client_id": client_id, "accuracy": acc, "loss": loss},
                    )
                    ...
                    if update_result.get("triggered") and update_result.get("aggregate"):
                        ...
                        if agg_result:
                            await fl_server.group_manager.broadcast_to_group(
                                group.group_id,
                                {
                                    "type": "model_update",
                ...
                try:
                    ...
                    if client_id in group.clients:
                        ...
                        group.clients[client_id]["local_accuracy"] = metrics.get(
                            "train_accuracy", 0
                        )
                        group.clients[client_id]["local_loss"] = metrics.get("train_loss", 0)
                        logger.debug(
                            f"Updated client {client_id} metrics: acc={metrics.get('train_accuracy', 0):.4f}, loss={metrics.get('train_loss', 0):.4f}"
                        )
