...
class HFVisionClassifier(nn.Module):
    ...
    def __init__(self, base_model: nn.Module, num_classes: int = 10):
        ...
        if hidden_size is None and hasattr(base_model, "vision_model"):
            hidden_size = getattr(
                getattr(base_model.vision_model, "config", None), "hidden_size", None
            )
...
class FederatedClient:
    ...
    def __init__(self, server_url: str, client_id: str, config: dict[str, Any]):
        ...
        self.local_client: LocalClient | None = None
        self.current_global_version = 0
        self.group_model_id: str | None = None
        self.group_model_info: dict[str, Any] | None = None
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if data.get("status") == "registered":
                ...
                self.logger.info(
                    f"Connected to server as {self.client_id} in group {data.get('group_id')}"
                )
                return True
    ...
    async def _handle_message(self, message: dict[str, Any]):
        ...
        if msg_type == "model_update":
            ...
            if message.get("config"):
                self.config.update(
                    {"client": {**self.config.get("client", {}), **message["config"]}}
                )
                ...
                self.config["client"].update(
                    {
                        "local_epochs": cfg.get(
                            "local_epochs", self.config["client"].get("local_epochs", 2)
                        ),
                        "batch_size": cfg.get(
                            "batch_size", self.config["client"].get("batch_size", 32)
                        ),
...
class RESTClient:
    ...
    def register_client(self, client_id: str) -> dict:
        ...
        response = self.session.post(
            f"{self.server_url}/api/clients/register", json={"client_id": client_id}
        )
    ...
    def upload_update(self, client_id: str, update: dict) -> dict:
        ...
        response = self.session.post(
            f"{self.server_url}/api/clients/{client_id}/update", json=update
        )
        return response.json()
...
def main():
    ...
    parser.add_argument(
        "--client-id", type=str, default=None, help="Client ID (auto-generated if not provided)"
    )

    # Group authentication
    parser.add_argument("--group-id", type=str, default="group_a", help="Group ID to join")
    parser.add_argument(
        "--join-token", type=str, default=None, help="Join token for group authentication"
    )
    ...
    parser.add_argument("--data-samples", type=int, default=None, help="Number of data samples")
    ...
    parser.add_argument("--password", type=str, default=None, help="Password for auto-login")
    ...
    # Mode
    parser.add_argument(
        "--mode", choices=["websocket", "rest"], default="websocket", help="Connection mode"
    )
    ...
    if args.mode == "websocket":
        client = FederatedClient(server_url=args.server, client_id=args.client_id, config=config)
